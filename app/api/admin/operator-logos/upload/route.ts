...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Validate file extension / MIME type (.png format only)
    const filename = (file.name || '').toLowerCase()
    const mimeType = (file.type || '').toLowerCase()
    const isPng = filename.endsWith('.png') || mimeType === 'image/png'

    if (!isPng) {
      return NextResponse.json({ error: 'Invalid file format. Only PNG (.png) images are allowed.' }, { status: 400 })
    }
    ...
    // 3. Upload to project Supabase storage 'operator-logos' bucket
    ...
    // 4. Update database record in operator_logos table
