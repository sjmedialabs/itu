...
export async function POST(req: Request) {
  try {
    ...
    // 5. Send email with reset URL / OTP via Resend/SMTP/Dev logger
    ...
    const exposeDev = shouldExposeDevOtp()
    return NextResponse.json({ ok: true, message: 'Password reset info sent successfully', ...(exposeDev ? { token } : {}) })
  } catch (e) {
