// Ding Connect Billing File Parser
...
export class DingParser implements ISupplierParser {
  parse(content: string, filename?: string): NormalizedSupplierRow[] {
    ...
    const idxTxId = resolveColumnIndex(headers, ['transactionid', 'dingid', 'txid', 'transaction_id', 'id']);
    const idxProvRef = resolveColumnIndex(headers, ['providerreference', 'providerref', 'reference', 'ref', 'dingreference']);
    ...
    const config = RECONCILIATION_CONFIG.suppliers.ding;
