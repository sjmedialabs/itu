-- Migration: Fix wallet recharge trigger to only debit the user_wallet_balance portion for recharges
  ...
  IF NEW.status = 'completed' AND (TG_OP = 'INSERT' OR OLD.status IS NULL OR OLD.status <> 'completed') THEN
    IF NEW.user_id IS NOT NULL THEN
      ...
      ELSIF NEW.type = 'recharge' THEN
        -- Check if it was funded by wallet or card
        BEGIN
          used_wallet_val := (NEW.metadata->>'used_wallet_balance')::numeric;
          IF used_wallet_val IS NOT NULL THEN
            net_amount := -used_wallet_val;
          ELSIF NEW.metadata->>'razorpay_payment_id' = 'wallet' THEN
            net_amount := -NEW.amount;
          ELSE
            -- Funded entirely by card, do not debit the wallet
            net_amount := 0;
          END IF;
